const STEPS = [
  ...
  },
  {
    ...
    label: "Win-Back",
    description:
      "A clean prompt offers them a second destination — a different offer, product, or channel — before they go.",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ width: 20, height: 20 }}>
        <path
          strokeLinecap="round"
...
export function TrafficRecovery() {
  return (
    <section aria-labelledby="win-back-heading" style={{ maxWidth: 1140, margin: "0 auto", padding: "72px 24px" }}>
      <div style={{ textAlign: "center", marginBottom: 40 }}>
        ...
        <p style={{ color: "#9A9A9A", fontSize: 17, margin: "0 auto", maxWidth: 560, lineHeight: 1.6 }}>
          Most visitors who leave are gone forever. Win-Back gives you a second chance — legitimately, without dark
          patterns.
        </p>
      </div>
      ...
      >
        {STEPS.map((step) => (
          ...
          >
            ...
            <h3 style={{ fontSize: 15, fontWeight: 600, color: "#0A0A0A", margin: "0 0 6px" }}>{step.label}</h3>
            <p style={{ fontSize: 13.5, lineHeight: 1.55, color: "#5a5a5a", margin: 0 }}>{step.description}</p>
          </div>
        ))}
      </div>
    </section>
