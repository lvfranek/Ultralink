...
type LocalPageState = Pick<Page, "title" | "bio" | "avatar_url" | "avatar_style" | "active_badge" | "slug">;
...
export function PageBuilder({ page, initialLinks, initialSocials, effectivePlan, userId, siteUrl }: PageBuilderProps) {
  ...
  const { slugOk, slugError, checking } = useSlugAvailability(local.slug, {
  ...
  const handlePresetApply = useCallback(
    async (key: PresetKey, linkStyle: LinkStyle) => {
      setLinks((prev) =>
        prev.map((l) => ({
          ...l,
          ...
          >
            <div className="flex-1 min-w-0 flex items-center gap-1.5">
              ...
              <button
                ...
                onMouseLeave={(e) => {
                  if (!copied) e.currentTarget.style.color = "#9A9A9A";
                }}
                aria-label="Copy URL"
              >
                {copied ? (
